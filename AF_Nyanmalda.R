...
dfriket_nyanmalda1664_kon <- fsokande_skapa_tab(kon = TRUE)

# Ladda hem data för arbetslösa i alla län 16-64 år fördelat på kön
dfnyanmalda1664_alla_lan_kon <- fsokande_extr_data_lan(kon = TRUE)
...
dfriket_nyanmalda1624 <- fsokande_skapa_tab(kon = FALSE)


# Ladda hem data för arbetslösa i riket i åldern 60
retry(fsokande_valj_alder(6))
dfriket_nyanmalda60 <- fsokande_skapa_tab(kon = FALSE)
...
dfriket_nyanmalda1664_sv <- fsokande_skapa_tab(kon = FALSE)

# Välj födda i övriga Europa
retry(fsokande_valj_fodelseland(2))
dfriket_nyanmalda1664_ovr_eur <- fsokande_skapa_tab(kon = FALSE)

# Välj födda i övriga Europa
retry(fsokande_valj_fodelseland(3))
dfriket_nyanmalda1664_ovr_varlden <- fsokande_skapa_tab(kon = FALSE)
...
dfriket_nyanmalda_funk <- fsokande_skapa_tab(kon = FALSE)
...
dfregion_kom_nyanmalda1664_kon <- fsokande_extr_data_kommuner(ant_kommuner, kon = TRUE)
...
dfregion_nyanmalda_funk <- fsokande_skapa_tab(kon = FALSE)

# Ladda hem tabell för funktionshindrade i länets kommuner 
# dfregion_kom_nyanmalda_funk <- fsokande_extr_data_kommuner(ant_kommuner, kon = FALSE)
...
dfregion_nyanmalda1624 <- fsokande_skapa_tab(kon = FALSE)

# Ladda hem data för regionens alla kommuner i åldern 16-24. Tar tid!
# dfregion_kom_nyanmalda1624 <- fsokande_extr_data_kommuner(ant_kommuner, kon = FALSE)
...
# Byt ålder till 60+ år (item nr 1 i Ålder-menyn)
retry(fsokande_valj_alder(6))

# Ladda hem data för arbetslösa i åldern 60+ år
dfregion_nyanmalda60 <- fsokande_skapa_tab(kon = FALSE)
...
retry(fsokande_valj_fodelseland(1))
dfregion_nyanmalda1664_sv <- fsokande_skapa_tab(kon = FALSE)

# Välj födda i övriga Europa
retry(fsokande_valj_fodelseland(2))
dfregion_nyanmalda1664_ovr_eur <- fsokande_skapa_tab(kon = FALSE)

# Välj födda utanför Europa
retry(fsokande_valj_fodelseland(3))
dfregion_nyanmalda1664_ovr_varlden <- fsokande_skapa_tab(kon = FALSE)
...
dfriket_nyanmalda1664_kon <- dfriket_nyanmalda1664_kon[, !names(dfriket_nyanmalda1664_kon) %in% c("etab","fland")] 
dfnyanmalda1664_alla_lan_kon <- dfnyanmalda1664_alla_lan_kon [, !names(dfnyanmalda1664_alla_lan_kon ) %in% c("etab","fland")]
#dfregion_kom_nyanmalda1664_kon <- dfregion_kom_nyanmalda1664_kon [, !names(dfregion_kom_nyanmalda1664_kon ) %in% c("etab","fland")]

dfnyanmalda1664_kon <- bind_rows(dfriket_nyanmalda1664_kon,
                                    dfnyanmalda1664_alla_lan_kon,
                                    dfregion_kom_nyanmalda1664_kon) %>%    
    separate(region,
...
dfriket_nyanmalda1624 <- dfriket_nyanmalda1624[, !names(dfriket_nyanmalda1624) %in% c("etab","fland")] 
dfriket_nyanmalda60 <- dfriket_nyanmalda60 [, !names(dfriket_nyanmalda60) %in% c("etab","fland")]
# dfregion_kom_nyanmalda1624 <- dfregion_kom_nyanmalda1624 [, !names(dfregion_kom_nyanmalda1624) %in% c("etab","fland")]
dfregion_nyanmalda60 <- dfregion_nyanmalda60 [, !names(dfregion_nyanmalda60) %in% c("etab","fland")]
dfregion_nyanmalda1624 <- dfregion_nyanmalda1624 [, !names(dfregion_nyanmalda1624 ) %in% c("etab","fland")]

##### Lägg ihop Öppet arbetslösa unga i en dataframe 
dfnyanmalda1624 <- bind_rows(dfriket_nyanmalda1624,
                             dfriket_nyanmalda60,
#                         dfregion_kom_nyanmalda1624,
                            dfregion_nyanmalda60,
                         dfregion_nyanmalda1624) %>%
    separate(region,
             into = c("region_kod", "region"), sep = "\\s", extra = "merge") %>%
...
#Säkerställ att inte oönskade kolumner kommer med
dfregion_nyanmalda1664_sv <- dfregion_nyanmalda1664_sv[, !names(dfregion_nyanmalda1664_sv)=="etab"] 
dfregion_nyanmalda1664_ovr_eur <- dfregion_nyanmalda1664_ovr_eur [, !names(dfregion_nyanmalda1664_ovr_eur)=="etab"]
dfregion_nyanmalda1664_ovr_varlden <- dfregion_nyanmalda1664_ovr_varlden [, !names(dfregion_nyanmalda1664_ovr_varlden )=="etab"]
dfriket_nyanmalda1664_sv <- dfriket_nyanmalda1664_sv [, !names(dfriket_nyanmalda1664_sv )=="etab"]
dfriket_nyanmalda1664_ovr_eur <- dfriket_nyanmalda1664_ovr_eur [, !names(dfriket_nyanmalda1664_ovr_eur )=="etab"]
dfriket_nyanmalda1664_ovr_varlden <- dfriket_nyanmalda1664_ovr_varlden [, !names(dfriket_nyanmalda1664_ovr_varlden )=="etab"]


dfnyanmalda_fland <- bind_rows(dfregion_nyanmalda1664_sv,
                           dfregion_nyanmalda1664_ovr_eur,
                           dfregion_nyanmalda1664_ovr_varlden,
                           dfriket_nyanmalda1664_sv,
                           dfriket_nyanmalda1664_ovr_eur,
                           dfriket_nyanmalda1664_ovr_varlden) %>%
    separate(region,
             into = c("region_kod", "region"), sep = "\\s", extra = "merge") %>%
    pivot_longer(cols = 2:4, names_to = "ar", values_to = "antal") %>%
    filter(!is.na(antal))
...
#Säkerställ att inte oönskade kolumner kommer med
dfriket_nyanmalda_funk <- dfriket_nyanmalda_funk[, !names(dfriket_nyanmalda_funk) %in% c("etab","fland")] 

dfnyanmalda_funk <- bind_rows(dfriket_nyanmalda_funk,
#                         dfregion_kom_nyanmalda_funk,
                         dfregion_nyanmalda_funk
                                            ) %>%
    separate(region,
             into = c("region_kod", "region"), sep = "\\s", extra = "merge") %>%
    pivot_longer(cols = 2:4, names_to = "ar", values_to = "antal") %>%
...
#sokvag1 <- "C:\\Users\\pmo\\OneDrive - Högskolan Dalarna\\Auto AF\\Uttag\\AF Nyinskrivna uttag.xlsx"
# Sökvägen på hemmadatorn
#sokvag2 <- "C:\\Users\\Administratör\\OneDrive - Region Dalarna\\Auto AF\\Uttag\\AF Nyinskrivna uttag.xlsx"
...
writexl::write_xlsx(list(Nyinskrivna_1624 = dfnyanmalda1624,
                         Nyinskrivna_fland = dfnyanmalda_fland,
                         Nyinskrivna_kon = dfnyanmalda1664_kon,
                         Nyinskrivna_funk = dfnyanmalda_funk
                         ),
                    path = sokvag)
